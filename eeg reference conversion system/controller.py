...
class MY_Controller:
    ...
    def quick_save_slot(self):
        if self.flag1 == 1 and self.flag2 == 1:
            ...
            if self.model.reference == 0:
                after_path = self.model.file_path[0:-4] + '_SET(' + self.channel + ')_raw.fif'
        ...
        if self.flag1 == 1 and self.flag2 == 1:
            ...
            if self.model.reference == 0:
                save_path, ok = file_dialog2.getSaveFileName(self.view, '选择保存路径',
                                                             self.model.file_path[0:-4] + '_SET(' + self.channel + ')_raw.fif',
                                                             'fif(*.fif)')
                if ok:
    ...
    def details_slot1(self):
        if self.flag1 == 1:
            with mne.viz.use_browser_backend("matplotlib"):
                fig = self.model.raw.plot(n_channels=len(self.model.raw))
    ...
    def details_slot2(self):
        if self.flag1 == 1:
            if self.flag2 == 1:
                with mne.viz.use_browser_backend("matplotlib"):
                    fig = self.model.raw_after.plot(n_channels=len(self.model.raw))
                fig.subplots_adjust(top=0.9)
                if self.model.reference == 0:
                    title = "Single electrode(" + self.channel + ")"
                elif self.model.reference == 1:
                    title = "LM"
                elif self.model.reference == 2:
                    title = "Average"
